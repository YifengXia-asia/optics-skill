...
| 状态码/现象 | 典型原因 | 处理 |
|---|---|---|
| `PREFLIGHT=FAIL` | Python 包、bash/timeout、MPI、VASP、oneAPI 路径缺失；平台不支持；输入文件为空；波长窗口或数值参数非法 | 先执行 `python -c "import sys; print(sys.executable)"`、`command -v mpirun`、`command -v timeout`、`test -x /绝对路径/vasp_std`；补齐依赖或改配置后重跑。Windows 可用 `--postprocess-only`，但不能运行 VASP |
