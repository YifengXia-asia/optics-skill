#!/usr/bin/env python3
"""Read-only environment, configuration, and input preflight for v0.7."""
...
POSTPROCESS_MODULES = ("yaml", "numpy", "pandas", "matplotlib", "lxml")
...
def _runtime_text(config: dict, command: str) -> str:
    setup = str(config["environment"].get("oneapi_setup", "") or "")
    parts = []
    if setup:
...
def _check_mpi_compatibility(config: dict) -> list[str]:
    """Detect the common Open MPI launcher + Intel MPI VASP mismatch."""
    ...
    selected = gamma_bin if gamma_only and bool(env.get("auto_select_gamma", True)) and gamma_bin else str(env.get("vasp_bin", "vasp_std"))
    vasp = shlex.quote(selected)
    launcher_text = _runtime_text(config, f"{launcher} --version 2>&1 | head -8").lower()
    linked_text = _runtime_text(config, f"ldd {vasp} 2>&1").lower()
    launcher_vendor = (
        "intel" if "intel(r) mpi" in launcher_text
        else "openmpi" if "open mpi" in launcher_text
        else "unknown"
    )
    linked_vendor = (
        "intel" if "/oneapi/mpi/" in linked_text or "libmpi.so.12" in linked_text
        else "openmpi" if "openmpi" in linked_text or "libmpi.so.40" in linked_text
        else "unknown"
    )
    if launcher_vendor != "unknown" and linked_vendor != "unknown" and launcher_vendor != linked_vendor:
        return [
            f"MPI vendor mismatch: launcher={launcher_vendor}, VASP-linked={linked_vendor}; "
            "use the launcher from the same MPI implementation as vasp_std"
        ]
    return []
...
def main() -> int:
    ...
    if parameters.get("response", "density-density") != "density-density":
        problems.append("parameters.response must be density-density in adaptive-v0.7")
    if parameters.get("parameter_policy", "preserve-user") not in {"preserve-user", "skill-only"}:
        problems.append("parameters.parameter_policy must be preserve-user or skill-only")
    metal_mode = str(run.get("metal_optics_mode", "stop"))
    if metal_mode not in {"stop", "interband-only", "drude"}:
        problems.append("run.metal_optics_mode must be stop, interband-only, or drude")
    if metal_mode == "drude":
        try:
            if float(parameters.get("loptics", {}).get("wplasmai", 0.0)) <= 0:
                problems.append("Drude mode requires parameters.loptics.wplasmai > 0 eV")
        except (TypeError, ValueError):
            problems.append("parameters.loptics.wplasmai must be numeric")
    try:
    ...
    if not args.postprocess_only:
        ...
        gamma_bin = str(env.get("vasp_gamma_bin", "") or "")
        if gamma_bin and not _find_executable(gamma_bin):
            problems.append(f"Executable not found: environment.vasp_gamma_bin={gamma_bin!r}")
        ...
        if not problems:
            problems.extend(_check_mpi_compatibility(config))
    ...
    grid = ((config.get("_metadata") or {}).get("kpoints") or {}).get("grid") or []
    gamma_bin = str(config["environment"].get("vasp_gamma_bin", "") or "")
    gamma_only = len(grid) == 3 and all(int(value) == 1 for value in grid)
    selected_vasp = gamma_bin if gamma_only and bool(config["environment"].get("auto_select_gamma", True)) and gamma_bin else str(config["environment"].get("vasp_bin", "vasp_std"))
    print(
        f"PREFLIGHT=PASS;MODE={mode};PLATFORM={platform.system()};"
        f"PYTHON={sys.executable};INPUT_DIR={input_dir};VASP_SELECTED={selected_vasp}"
    )
    return 0
