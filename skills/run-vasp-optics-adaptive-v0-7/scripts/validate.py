#!/usr/bin/env python3
"""只读验证材料类型、自适应决策和光学输出（adaptive-v0.7）。
...
def main() -> int:
    ...
    xmin = float(postprocess.get("plot_min_nm", 300.0))
    xmax = float(postprocess.get("plot_max_nm", 2500.0))
    ...
    required = [
        ...
        f"{prefix}_optics_metadata.json",
    ]
    ...
    classification = json.loads((run_dir / "system_classification.json").read_text(encoding="utf-8"))
    optical_metadata = json.loads((run_dir / f"{prefix}_optics_metadata.json").read_text(encoding="utf-8"))
    if classification.get("structure_class") != optical_metadata.get("structure_class"):
        fail("Classification and optical metadata disagree on structure_class.")
    if classification.get("electronic_class") != optical_metadata.get("electronic_class"):
        fail("Classification and optical metadata disagree on electronic_class.")
    if classification.get("structure_class") != "bulk-3d-candidate":
        if optical_metadata.get("intrinsic_bulk_optical_constants") is not False:
            fail("Non-bulk results must be labelled as non-intrinsic supercell values.")
    ...
    energy_pngs = ["epsilon1", "epsilon2", "n", "k", "alpha", "R", "loss", "sigma1"]
    ...
    mc_v01_wavelength_pngs = [
        ...
        "loss_vs_wavelength", "sigma1_vs_wavelength",
    ]
    legacy_wavelength_pngs = [
        "alpha_vs_wavelength", "R_vs_wavelength", "n_vs_wavelength",
        "k_vs_wavelength", "loss_vs_wavelength", "sigma1_vs_wavelength",
    ]
    mc_v01_extra_pngs = [
        f"alpha_{int(xmin)}_{int(xmax)}nm.png",
        f"alpha_{int(xmin)}_{int(xmax)}nm_log.png",
        f"R_{int(xmin)}_{int(xmax)}nm.png",
    ]
    legacy_extra_pngs = [
        f"alpha_{int(xmin)}_{int(xmax)}nm.png",
        f"R_{int(xmin)}_{int(xmax)}nm.png",
    ...
    print(
        f"VALIDATION=PASS;MATERIAL={run.get('material', prefix)};ROWS={len(rows)};"
        f"STRUCTURE_CLASS={classification.get('structure_class')};"
        f"ELECTRONIC_CLASS={classification.get('electronic_class')};"
        f"INTRINSIC_BULK={optical_metadata.get('intrinsic_bulk_optical_constants')};"
        f"RESPONSE={selected_response};RESPONSES={sorted(responses)}"
    )
    return 0
