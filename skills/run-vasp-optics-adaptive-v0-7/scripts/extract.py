...
def derive(energy: np.ndarray, eps1: np.ndarray, eps2: np.ndarray, wavelength_constant: float):
    ...
    wavelength_nm[positive] = wavelength_constant / energy[positive]
    loss = np.divide(
        eps2,
        eps1 * eps1 + eps2 * eps2,
        out=np.zeros_like(eps2),
        where=(eps1 * eps1 + eps2 * eps2) > 1e-30,
    )
    sigma1 = EPS0 * omega * eps2
    return n, kappa, alpha_cm, reflectivity, wavelength_nm, loss, sigma1
...
def main() -> int:
    ...
    wavelength_constant = float(
        config.get("postprocess", {}).get("wavelength_constant_nm_eV", 1239.841984)
    )
    n, kappa, alpha_cm, reflectivity, wavelength_nm, loss, sigma1 = derive(
        energy, eps1_avg, eps2_avg, wavelength_constant
    )
    ...
    rows = np.column_stack([
        ...
        eps1_avg, eps2_avg, n, kappa, alpha_cm, reflectivity, loss, sigma1,
    ])
    header = [
    ...
    save_plot(energy, loss, r"Loss function Im$[-1/\epsilon]$",
              outdir / f"{prefix}_loss.png", f"{material} Energy-Loss Function")
    save_plot(energy, sigma1, r"Optical conductivity $\sigma_1$ (S m$^{-1}$)",
              outdir / f"{prefix}_sigma1.png", f"{material} Real Optical Conductivity")

    classification_path = optics_dir / "system_classification.json"
    classification = {}
    if classification_path.is_file():
        classification = json.loads(classification_path.read_text(encoding="utf-8"))
    intrinsic = classification.get("structure_class") == "bulk-3d-candidate"
    metadata_path = outdir / f"{prefix}_optics_metadata.json"
    metadata_path.write_text(
        json.dumps(
            {
                "material_label": material,
                "response": response,
                "structure_class": classification.get("structure_class", "unknown"),
                "electronic_class": classification.get("electronic_class", "unknown"),
                "intrinsic_bulk_optical_constants": intrinsic,
                "normalization": "3d-cell-volume" if intrinsic else "3d-supercell-volume-dependent",
                "metal_optics_mode": run.get("metal_optics_mode", "stop"),
                "limitations": [
                    "Independent-particle LOPTICS; no excitons or local-field correction.",
                    *( [] if intrinsic else ["Non-bulk values depend on vacuum/cell volume and are not intrinsic bulk constants."] ),
                ],
            },
            ensure_ascii=False,
            indent=2,
    ...
    print(f"METADATA={metadata_path};INTRINSIC_BULK={intrinsic}")
    return 0
